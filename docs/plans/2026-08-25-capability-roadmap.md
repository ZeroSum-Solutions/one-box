...
| Visual typography specimens | Shipped | The token review renders every canonical family in its declared role with the relevant hierarchy, weights, sizes, line heights, and tracking. Preloaded families are labeled as loaded; other valid family names are applied but explicitly disclose that the browser may show its fallback. |
