...
| Visual typography specimens | Partial | The token review renders every canonical family in its declared role with the relevant hierarchy, weights, sizes, line heights, and tracking. It proves preloaded families only at weights whose files are present, and explicitly flags unavailable families or weights instead of pretending a fallback or synthetic weight is the proposal. Rendering every valid arbitrary family requires the font-source, rights, and pinned-file work in Slice 1. |
| Color, spacing, radius, border, shadow, overlay, and stacking-layer specimens | Shipped | Token review shows every canonical value in a bounded role example before its technical table. |
| Hover, focus, selected, disabled, and error specimens | Shipped | The token review renders every canonical component-state token with its actual state name and value, without moving client colors into OneBox shell chrome. |
| Desktop, tablet, phone, keyboard, focus, touch, overflow, motion, and Axe verification | Shipped | `scripts/e2e/evidence-review.mjs` covers the shared review surface at 1440, 768, and 390 pixels, measures every visible interactive target plus attachment-present disclosure/removal controls at mobile widths, checks keyboard disclosure, focus, Escape stability, overflow, console and page errors, script-blocked mode, reduced motion, both canonical computed font roles, forced upload expiry and recovery, attachment removal and persistence, reference-feedback exhaustion recovery, and Axe serious/critical findings. Unit coverage exercises every artifact reader and the reference-selection review contract. |
