...
export function TokenControls({ runId, onMutationComplete, onPreview }: { runId: string; onMutationComplete: () => void; onPreview: (token: string, value: string) => void }) {
  ...
  return <div className="token-controls">
    ...
    {selected && <>
      ...
      <div className="token-actions"><button type="button" className="btn-ghost" disabled={pending || !selected.editable} onClick={() => void act("preview")}>Preview change</button><button type="button" className="btn-primary" disabled={pending || !preview} onClick={() => void act("apply")}>Apply token</button><button type="button" className="btn-ghost" disabled={pending || !data.canRevert} onClick={() => void act("revert")}>Revert last token</button></div>
    </>}
